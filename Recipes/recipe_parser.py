    ...
    def select_best_match_with_gpt(self, ingredient: Dict, matches: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            if not scored_matches:
                logger.info(f"No matches with sufficient name similarity for {ingredient['item']}")
                return None
...
If none are suitable due to unit or cost, return 0."""
            ...
            logger.error(f"GPT matching error: {e}")
            return None
    ...
    async def get_best_match(self, ingredient: Dict) -> Optional[Dict]:
        ...
        if inventory_match and inventory_match.get('similarity', 0) >= 0.5:
            logger.info(f"Found inventory match for {ingredient['item']} with {inventory_match['similarity']*100:.1f}% similarity")
            return inventory_match
            
        # If no inventory match, try retail prices
        logger.info(f"No good inventory match for {ingredient['item']}, searching retail prices...")
        try:
            ...
            if best_price:
                ...
                logger.info(f"Found retail match for {ingredient['item']} from {best_retailer}")
                return retail_match
        except Exception as e:
            logger.error(f"Error in retail price search: {e}")
            
        logger.warning(f"No matches found for {ingredient['item']}")
        return None
...
class UnitConverter:
    ...
    def convert(self, amount: Decimal, from_unit: str, to_unit: str) -> Decimal:
        ...
        try:
            ...
            logger.warning(f"Pint conversion failed: {e}")
...
5. Topping Instructions:
   ...
   - Maintain sequence of steps"""
        ...
        try:
            recipes_data = json.loads(response.choices[0].message.content)
            logger.info(f"Successfully extracted {len(recipes_data['recipes'])} recipes")
            return recipes_data
        except json.JSONDecodeError as e:
            logger.error(f"JSON parsing error: {e}")
            raise
...
async def main():
    """Run the recipe cost calculator."""
    try:
        # Setup clients
        ...
        # Process Excel file
        excel_path = "C:/Users/rahul/Downloads/CEPC_Menu Item Breakdown_Food Costing (1).xlsx"
        
        if not os.path.exists(excel_path):
            raise FileNotFoundError(f"Excel file not found: {excel_path}")
        
        # Extract and process recipes using the new Excel function
        recipes_data = extract_recipes_from_excel(excel_path, openai_client)
        ...
        # Generate output path based on input Excel name
        excel_name = os.path.splitext(os.path.basename(excel_path))[0]
        output_dir = os.path.dirname(excel_path)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(output_dir, f"{excel_name}_costs_{timestamp}.xlsx")
